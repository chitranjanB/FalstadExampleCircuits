...
abstract class GateElm extends CircuitElm {
    ...
    boolean lastOutput;
    boolean hasPulldown;
    // Do the inputs have a fictitious pull-down resistor, i.e. do they require some current?
    // If true, put a 1 MOhm resistor to ground
    boolean hasPullup;
    ...
    void stamp() {
        ...
        if (hasPulldown) {
            int i;
            for (i = 0; i < inputCount; i++) {
                sim.stampResistor(nodes[i], 0, 1e6);
                // If hasPulldown, put a 1MOhm resistor between all inputs and ground
            }
        }
    }
    ...
    public EditInfo getEditInfo(int n) {
        ...
        if (n == 1) {
            ...
            ei.checkbox = new Checkbox("Require input current", hasPulldown);
            return ei;
        }
        return null;
    }
    ...
    public void setEditValue(int n, EditInfo ei) {
        ...
        if (n == 1) {
            hasPulldown = ei.checkbox.getState();
        }
    }
